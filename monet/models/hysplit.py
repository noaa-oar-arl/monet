...
class ModelBin(object):
  ...
  def readfile(self, filename, drange, verbose, century): 
     ...
     self.dset = None
     atthash = {}    #dictionary which will be turned into the dset attributes.
     ahash = {}
     fp = open(filename, 'rb') 
     ...
     atthash['Meteorological Model ID']  = hdata1['model_id'][0].decode('UTF-8')
     self.sourcedate=[]
     self.slat=[]
     self.slon=[]
     self.sht=[]
     ...
     for n in  range(0,nstartloc):
        ...
        atthash['Starting Locations'].append((hdata2['s_lat'][n], hdata2['s_lon'][n])  )
        ...
        sourcedate = (datetime.datetime(century+hdata2['r_year'][n] , \
     ...
     while testf:
        ...
        for lev in range(hdata4a['nlev'][0]):
            ...
            for pollutant in range(hdata5a['pollnum'][0]):
                ...
                atthash['Species ID'].append(hdata8a['poll'][0].decode('UTF-8'))
                ...
                hdata8c=np.fromfile(fp,dtype=rec8c, count=1) #This is just padding.
