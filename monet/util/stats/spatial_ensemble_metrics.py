...
def FSS(obs, mod, window=3, threshold=None):
    ...
    if xr is not None and isinstance(obs, xr.DataArray) and isinstance(mod, xr.DataArray):
        ...
        obs_frac = xr.DataArray(uniform_filter(obs_bin, window, mode='nearest'), dims=obs.dims, coords=obs.coords)
        mod_frac = xr.DataArray(uniform_filter(mod_bin, window, mode='nearest'), dims=mod.dims, coords=mod.coords)
...
def SAL(obs, mod, threshold=None):
    ...
    S = 2 * (max_mod/sum_mod - max_obs/sum_obs) / (max_mod/sum_mod + max_obs/sum_obs) if sum_mod > 0 and sum_obs > 0 else np.nan
    # Location
    def centroid(X):
        result = ndi.label(X >= threshold)
        if isinstance(result, tuple):
