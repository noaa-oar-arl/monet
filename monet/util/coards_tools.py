...
def add_cf_attributes(ds, **kwargs):
    ...
    **kwargs : dict
    ...
    if isinstance(result, xr.Dataset):
        ...
        if (
            "longitude" in result.coords
            and "standard_name" not in result.longitude.attrs
        ):
        ...
        if (
            "latitude" in result.coords
            and "standard_name" not in result.coords["latitude"].attrs
        ):
        ...
        if (
            "longitude" in result.coords
            and "standard_name" not in result.coords["longitude"].attrs
        ):
                    ...
                    try:
                        ...
                        if lat_constant_along_x and lon_constant_along_y:
                            ...
                            if isinstance(result, xr.Dataset):
                                ...
                                result["latitude_2d"].attrs["standard_name"] = (
                                    "latitude"
                                )
                                result["latitude_2d"].attrs["units"] = "degrees_north"
                                result["longitude_2d"].attrs["standard_name"] = (
                                    "longitude"
                                )
    ...
    if "time" in result.coords:
        ...
        if "units" not in result["time"].attrs:
            ...
            try:
                ...
                ref_date = pd.Timestamp(t0).replace(
                    hour=0, minute=0, second=0, microsecond=0
                )
                result["time"].attrs["units"] = (
