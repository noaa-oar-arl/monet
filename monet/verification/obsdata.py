...
class StationData(object):
    ...
    def find_near(self, latlon, distance=100, sid='site_num', drange=None):
        """find all values in column sid which are within distance (km) of lat lon point.
    ...
    def write_datem(self, obscolumn='obs', dname = 'datemfile.txt', sitename='1', info=None, drange=None):
        ...
        dname is a string which is the name of the output file.
        sitename is a string. If it is the name of a column in the dataframe then 
        ...
        info is a string that will be written to the second line of the header.
        drange is list of two time stamp objects.
    ...
    def to_hdf(self, hdfname):
        #not working very well. hdf file too large.
        store = pd.HDFStore(hdfname)
        store.put('df', self.df) 
        #self.df.to_hdf(hdfname, 'key_to_store', format='fixed', mode='w')

    def from_hdf(self, hdfname):
        store = pd.HDFStore(hdfname)
        self.df = store['df']
    ...
    def latlonfilter(self, llcrnr, urcrnr):
        """llcrnr tuple (latitude, longitude)
           urcrnr tuple (latittude, longitude)
