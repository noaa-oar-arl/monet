...
class crn(object):
    def __init__(self):
        ...
        self.hcols = ['WBANNO', 'UTC_DATE', 'UTC_TIME', 'LST_DATE', 'LST_TIME', 'CRX_VN',
                      ...
                      'RH_AVG', 'RH_AVG_FLAG', 'SOIL_MOISTURE_5',
        ...
        self.dcols = ['WBANNO', 'LST_DATE', 'CRX_VN', 'LONGITUDE', 'LATITUDE', 'T_MAX', 'T_MIN', 'T_MEAN', 'T_AVG',
                      'P_CALC', 'SOLARAD', 'SUR_TEMP_TYPE', 'SUR_TEMP_MAX', 'SUR_TEMP_MAX',
                      'SUR_TEMP_MIN', 'SUR_TEMP_AVG', 'RH_MAX', 'RH_MIN', 'RH_AVG', 'SOIL_MOISTURE_5',
                      'SOIL_MOISTURE_10', 'SOIL_MOISTURE_20', 'SOIL_MOISTURE_50',
                      'SOIL_MOISTURE_100', 'SOIL_TEMP_5', 'SOIL_TEMP_10', 'SOIL_TEMP_20',
                      'SOIL_TEMP_50', 'SOIL_TEMP_100']
        self.shcols = ['WBANNO', 'UTC_DATE', 'UTC_TIME', 'LST_DATE', 'LST_TIME', 'CRX_VN', 'LONGITUDE', 'LATITUDE', 'T_MEAN',
                       'P_CALC', 'SOLARAD', 'SOLARAD_FLAG', 'SUR_TEMP_AVG', 'SUR_TEMP_TYPE', 'SUR_TEMP_FLAG', 'RH_AVG',
                       'RH_FLAG', 'SOIL_MOISTURE_5', 'SOIL_TEMP_5', 'WETNESS', 'WET_FLAG', 'WIND', 'WIND_FLAG']
    ...
    def load_file(self, url):
        ...
        if 'CRND0103' in url:
            ...
            cols = self.shcols
            df = pd.read_csv(url, delim_whitespace=True, names=cols, parse_dates={'time': ['UTC_DATE', 'UTC_TIME'],
                                                                                  'time_local': ['LST_DATE', 'LST_TIME']}, infer_datetime_format=True, na_values=navals)
        else:
            cols = self.hcols
            df = pd.read_csv(url, delim_whitespace=True, names=cols, parse_dates={'time': ['UTC_DATE', 'UTC_TIME'],
                                                                                  'time_local': ['LST_DATE', 'LST_TIME']}, infer_datetime_format=True, na_values=nanvals)
        return df
    ...
    def add_data(self, dates, daily=False, sub_hourly=False, download=False, latlonbox=None):
        ...
        if latlonbox is not None:  # get them all[latmin,lonmin,latmax,lonmax]
            mdf = self.monitor_df
            con = (mdf.LATITUDE >= latlonbox[0]) & (mdf.LATITUDE <= latlonbox[2]) & (mdf.LONGITUDE >= latlonbox[1]) & (mdf.LONGITUDE <= latlonbox[3])
        ...
        self.df = pd.melt(self.df, id_vars=keys, var_name='variable', value_name='obs')  # this stacks columns to be inline with MONET
