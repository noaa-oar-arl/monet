...
def get_surface_dust(cmaqobj):
    dustvars = array([ 'ASO4J','ASO4K','ANO3J','ANO3K','ACLJ','ACLK','ANH4J','ANAJ','ACAJ','AMGJ','AKJ','APOCJ','APNCOMJ','AECJ','AFEJ','AALJ','ASIJ','ATIJ','AMNJ','AH2OJ','AOTHRJ','ASOIL'])
...
def get_surface_pm25_dust(cmaqobj):
    pm25vars = array(['ASO4J','ANO3J','ACLJ','ANH4J','ANAJ','ACAJ','AMGJ','AKJ','APOCJ','APNCOMJ','AECJ','AFEJ','AALJ','ASIJ','ATIJ','AMNJ','AH2OJ','AOTHRJ'])
    keys = array(cmaqobj.variables.keys())
...
def get_surface_pm25(cmaqobj):
    ...
    pm25vars = array(['AALJ','AALK1J','AALK2J','ABNZ1J','ABNZ2J','ABNZ3J',
       ...
       'ATOL3J','ATRP1J','ATRP2J','AXYL1J','AXYL2J','AXYL3J','A25I','A25J','AORGAI','AORGAJ','AORGPAI','AORGPAJ','AORGBI','AORGBJ'])
    keys = array(cmaqobj.variables.keys())
    cmaqvars,temp= search_listinlist(keys,pm25vars)
    pm = zeros(cmaqobj.variables[keys[cmaqvars[0]]][:][:,0,:,:].squeeze().shape)
    for i in cmaqvars[1:]:
...
def interp_to_aqs_sites(cmaqvar,aqs,gridobj,dates):
    ...
    con = (aqs.datetime == dates[0]) & (aqs.Latitude.values > lat.min()) & (aqs.Latitude.values < lat.max()) & (aqs.Longitude.values > lon.min()) & (aqs.Longitude.values < lon.max())
    new = aqs[con]
    cmaq_val = pd.DataFrame(griddata((lon.flatten(),lat.flatten()),cmaqvar[0,:,:].flatten(),(new.Longitude.values,new.Latitude.values),method='nearest'),columns=['cmaq'],index=new.index)
    new = new.join(cmaq_val)
    for i,j in enumerate(dates[1:]):
        con = (aqs.datetime == j) & (aqs.Latitude.values > lat.min()) & (aqs.Latitude.values < lat.max()) & (aqs.Longitude.values > lon.min()) & (aqs.Longitude.values < lon.max())
        newt = aqs[con]
        cmaq_val = pd.DataFrame(griddata((lon.flatten(),lat.flatten()),cmaqvar[i,:,:].flatten(),(newt.Longitude.values,newt.Latitude.values),method='nearest'),columns=['cmaq'],index=newt.index)
...
def calc_stats(pandasobj):
    ...
    e1 = mstats.E1(pandasobj['Sample Measurement'].values,pandasobj['cmaq'].values)     # Modified Coefficient of Efficiency, E1
...
def calc_stats_daily(pandasobj):
    ...
    e1 = mstats.E1(pandasobj['Arithmetic Mean'].values,pandasobj['cmaq'].values)     # Modified Coefficient of Efficiency, E1
...
def compare_aqs_pm10(cmaqobj,gridobj,statecompare=True,spatial=True,scatter=False,time=False):
    ...
    if spatial:
        ...
        for index,i in enumerate(dates):
            ...
            m,c = cr.make_spatial_plot(cmaqvar,gridobj,dates[index],vmin=vmin,vmax=vmax,pick='cmaq_grids/basemap-cmaq_conus.p')
...
def compare_aqs_pm10_dust(cmaqobj,gridobj,statecompare=True,spatial=True,scatter=False,time=False):
    ...
    if spatial:
        ...
        for index,i in enumerate(dates):
            cmaqvar = cmaq[index,:,:].squeeze()
            m,c = cr.make_spatial_plot(cmaqobj,gridobj,dates[index],vmin=vmin,vmax=vmax,pick='cmaq_grids/basemap-cmaq_conus.p')
            cr.improve_spatial_scatter(improve,m,i.strftime('%Y-%m-%d %H:%M:%S'),'Sample Measurement',vmn=vmin,vmax=vmax)
...
def compare_aqs_ozone(cmaqobj,gridobj,statecompare=True,spatial=True,scatter=False,time=False):
    ...
    if spatial:
        ...
        for index,i in enumerate(dates):
            cmaqvar = cmaq[index,:,:].squeeze()
            m,c = cr.make_spatial_plot(cmaqobj,gridobj,dates[index],vmin=vmin,vmax=vmax,pick='cmaq_grids/basemap-cmaq_conus.p')
            cr.improve_spatial_scatter(improve,m,i.strftime('%Y-%m-%d %H:%M:%S'),'Sample Measurement',vmin=vmin,vmax=vmax)
