...
class aqs:
    ...
    def __init__(self):
        ...
        self.dates = [datetime.strptime('2014-06-06 12:00:00','%Y-%m-%d %H:%M:%S'),datetime.strptime('2014-06-06 13:00:00','%Y-%m-%d %H:%M:%S')]
        ...
        self.renameddcols = ['datetime_local', 'State_Code', 'County_Code', 'Site_Num',
                          ...
                          'Observation_Percent', 'Arithmetic_Mean', '1st_Max_Value',
    ...
    def retrieve_aqs_hourly_pm25_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
        ...
        df = pd.read_csv(filename,compression='gzip',parse_dates={'datetime':['Date GMT', 'Time GMT'],'datetime_local':["Date Local","Time Local"]},date_parser=dateparse)
        df.columns = self.renamedhcols
        df['SCS'] = array(df['State_Code'].values * 1.E7 + df['County_Code'].values * 1.E4 + df['Site_Num'].values,dtype='int32')
    ...
    def retrieve_aqs_hourly_pm10_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
    ...
    def retrieve_aqs_hourly_so2_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
    ...
    def retrieve_aqs_hourly_no2_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
    ...
    def retrieve_aqs_hourly_co_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
    ...
    def retrieve_aqs_daily_co_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
        ...
        df = pd.read_csv(filename,compression='gzip',parse_dates={'datetime_local':["Date Local"]},date_parser=dateparse)
    ...
    def retrieve_aqs_daily_pm10_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
    ...
    def retrieve_aqs_daily_so2_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
    ...
    def retrieve_aqs_daily_no2_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
    ...
    def retrieve_aqs_daily_pm25_data(self):
        import wget
        i = dates[0]
        year = i.strftime('%Y')
    ...
    def load_aqs_pm25_data(dates):
        ...
        aqs = pd.read_hdf(self.datadir+ '/' + self.datadir + '/'+'AQS_HOURLY_PM_25_88101_'+year+'.hdf')
        con = (aqs.datetime >= dates[0]) & (aqs.datetime <= dates[-1])
        aqs = aqs[con]
        aqs.index = arange(aqs.index.shape[0])
        return aqs

    def load_aqs_ozone_data(dates):
        year = dates[0].strftime('%Y_')
        aqs = pd.read_hdf(self.datadir+ '/' + self.datadir + '/'+'AQS_HOURLY_OZONE_44201_'+year+'.hdf')
        con = (aqs.datetime >= dates[0]) & (aqs.datetime <= dates[-1])
        aqs = aqs[con]
        aqs.index = arange(aqs.index.shape[0])
        return aqs
    ...
    def load_aqs_co_data(dates):
        year = dates[0].strftime('%Y_')
        aqs = pd.read_hdf(self.datadir+ '/' + self.datadir + '/'+'AQS_HOURLY_CO_42101_'+year+'.hdf')
        con = (aqs.datetime >= dates[0]) & (aqs.datetime <= dates[-1])
        aqs = aqs[con]
        aqs.index = arange(aqs.index.shape[0])
        return aqs

    def load_aqs_daily_pm25_data(dates):
        year = dates[0].strftime('%Y_')
    ...
    def load_aqs_daily_ozone_data(dates):
        year = dates[0].strftime('%Y_')
    ...
    def load_aqs_daily_pm10_data(dates):
        year = dates[0].strftime('%Y_')
    ...
    def load_aqs_daily_so2_data(dates):
        year = dates[0].strftime('%Y_')
    ...
    def load_aqs_daily_no2_data(dates):
        year = dates[0].strftime('%Y_')
    ...
    def load_aqs_daily_co_data(dates):
        year = dates[0].strftime('%Y_')
